	...
	class Category extends Admin_Controller {
		...
		if($id!=NULL || !empty($this->input->post('cate_id'))){
			...
			$this->mo_category->cate_id = $this->input->post('cate_id');
				...
				// $this->mo_category->cate_sub = $this->input->post('cate_sub', TRUE);
				// $this->mo_category->cate_main = $this->input->post('cate_main');
			...
			$this->mo_category->cate_id = $this->input->post('cate_id');
				...
				$gg = $this->mo_category->cate_sub;
				$this->mViewData['id_test2'] = $this->mo_category->get_sub_category($gg);
				$yyy = $this->mViewData['id_test2'];
				foreach($yyy as $rrr){
					$this->mViewData['id_test3'] = $rrr->cate_main;
				}
				$aa = $this->mViewData['id_test3'];
				if($aa==0){
					$this->mo_category->cate_main = $gg;
				}else{
					$this->mo_category->cate_main = $this->mViewData['id_test3'];
				}
